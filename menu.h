...
public:
	...
	*/
	...
	Checkbox	  static_multipoint_enable;
	Slider		  scale;
	Slider		  body_scale;
...
public:
	void init() {
		...
		static_multipoint_enable.setup(XOR("static multipoint"), XOR("static_multipoint_enable"));
		RegisterElement(&static_multipoint_enable);

		scale.setup("head scale", XOR("hitbox_scale"), 1.f, 100.f, false, 0, 90.f, 1.f, XOR(L"%"));
		scale.AddShowCallback(callbacks::IsMultipointOn);
		RegisterElement(&scale);

		body_scale.setup(XOR("body scale"), XOR("body_hitbox_scale"), 1.f, 100.f, true, 0, 50.f, 1.f, XOR(L"%"));
		body_scale.AddShowCallback(callbacks::IsMultipointOn);
		...
		nospread.setup(XOR("no spread"), XOR("nospread"));
		nospread.AddShowCallback(callbacks::IsConfigNS);
		RegisterElement(&nospread, 1);

		norecoil.setup(XOR("no recoil"), XOR("norecoil"));
...
public:
	void init() {
		...
		body_fake_offset_stand.setup("", XOR("body_fake_offset_stand"), -180.f, 180.f, false, 0, 0.f, 1.f, XOR(L""));
		body_fake_offset_stand.AddShowCallback(callbacks::IsAntiAimModeStand);
		body_fake_offset_stand.AddShowCallback(callbacks::HasStandYaw);
		body_fake_offset_stand.AddShowCallback(callbacks::IsFakeBodyCustomStand);
		RegisterElement(&body_fake_offset_stand);

		// walk.
		pitch_walk.setup(XOR("pitch"), XOR("pitch_walk"), { XOR("off"), XOR("down"), XOR("up"), XOR("random"), XOR("ideal"), XOR("emotion") });
		pitch_walk.AddShowCallback(callbacks::IsAntiAimModeWalk);
		RegisterElement(&pitch_walk);

		yaw_walk.setup(XOR("yaw"), XOR("yaw_walk"), { XOR("off"), XOR("direction"), XOR("jitter"), XOR("rotate"), XOR("delta offset"), XOR("crooked") });
		...
		dir_walk.setup(XOR("direction"), XOR("dir_walk"), { XOR("auto"), XOR("manual"), XOR("custom") });
		...
		// air.
		pitch_air.setup(XOR("pitch"), XOR("pitch_air"), { XOR("off"), XOR("down"), XOR("up"), XOR("random"), XOR("ideal"), XOR("emotion") });
		pitch_air.AddShowCallback(callbacks::IsAntiAimModeAir);
		RegisterElement(&pitch_air);

		yaw_air.setup(XOR("yaw"), XOR("yaw_air"), { XOR("off"), XOR("direction"), XOR("jitter"), XOR("rotate"), XOR("delta offset"), XOR("crooked") });
		...
		dir_air.setup(XOR("direction"), XOR("dir_air"), { XOR("auto"), XOR("manual"), XOR("custom") });
		...
		body_fake_air.setup(XOR("fake body"), XOR("body_fake_air"), { XOR("off"),XOR("custom"), XOR("random") });
		...
		body_fake_offset_air.setup("", XOR("body_fake_offset_air"), -180.f, 180.f, false, 0, 0.f, 1.f, XOR(L"�"));
		body_fake_offset_air.AddShowCallback(callbacks::IsAntiAimModeAir);
		body_fake_offset_air.AddShowCallback(callbacks::IsFakeBodyCustomAir);
		body_fake_offset_air.AddShowCallback(callbacks::AirHasYaw);
		RegisterElement(&body_fake_offset_air);

		// col2.
		fake_yaw.setup(XOR("fake yaw"), XOR("fake_yaw"), { XOR("off"), XOR("desync"), XOR("jitter"), XOR("float"), XOR("fake-flick"), XOR("jitter rotate"), XOR("rotate"), XOR("swap"), XOR("static backwards") });
		...
		fake_swap_range.setup("", XOR("fake_swap_range"), 10.f, 50.f, false, 0, 0.f, 5.f, XOR(L""));
		fake_swap_range.AddShowCallback(callbacks::IsFakeAntiAimSwap);
		RegisterElement(&fake_swap_range, 1);
...
public:
	void init() {
		...
		lby_update_color.setup(XOR("color"), XOR("lby_update_color"), colors::light_blue);
		...
		glow_enemy.setup(XOR("enemy color"), XOR("glow_enemy"), { 197, 78, 132 });
		RegisterElement(&glow_enemy, 1);

		glow_friendly.setup(XOR("friendly color"), XOR("glow_friendly"), { 197, 78, 132 });
		...
		chams_enemy_mat.setup(XOR("chams enemy material"), XOR("chams_enemy_mat"), { XOR("material"), XOR("flat"), XOR("metallic"), XOR("shaded"), XOR("glow") });
...
public:
	void init() {
		...
		on_shot_hitbox.setup(XOR("onshot hitbox"), XOR("on_shot_hitbox"));
		RegisterElement(&on_shot_hitbox);

		on_shot_hitbox_color.setup(XOR("onshot color"), XOR("on_shot_hitbox_color"), colors::white);
		RegisterElement(&on_shot_hitbox_color);

		on_shot_hitbox_time.setup(XOR("onshot hitbox time"), XOR("on_shot_hitbox_time"), 1.f, 10.f, true, 0, 1.f, 1.f, XOR(L"s"));
		RegisterElement(&on_shot_hitbox_time);
		...
		pen_crosshair.setup("penetration visualizer", XOR("pen_xhair"), { XOR("off"), XOR("2d crosshair"), XOR("2d rect"), XOR("2d circle"), XOR("3d rect") });
		...
		tracers_color.setup(XOR("grenade prediction color"), XOR("tracers_color"), colors::baby_blue);
		RegisterElement(&tracers_color, 1);
		...
		thirdperson_distance.setup("", XOR("thirdperson_distance"), 50.f, 200.f, false, 0, 150.f, 1.f, XOR(L"�"));
...
public:
	void init() {
		...
		RegisterElement(&slide_walk);

		autostrafe.setup(XOR("automatic strafe"), XOR("autostrafe"));
		RegisterElement(&autostrafe);

		movementstrafe.setup(XOR("movement key strafe"), XOR("movementstrafe"));
		RegisterElement(&movementstrafe);
		...
		manualdown.setup(XOR("manual backwards"), XOR("manual_down"));
		manualdown.SetToggleCallback(callbacks::ToggleDownManual);
		RegisterElement(&manualdown, 1);

		manualleft.setup(XOR("manual left"), XOR("manual_left"));
		manualleft.SetToggleCallback(callbacks::ToggleLeftManual);
		RegisterElement(&manualleft, 1);

		manualright.setup(XOR("manual right"), XOR("manual_right"));
		manualright.SetToggleCallback(callbacks::ToggleRightManual);
		RegisterElement(&manualright, 1);
...
public:
	void init() {
		...
		hitmarker.setup(XOR("hitmarker"), XOR("hitmarker"), { XOR("screen"), XOR("world"), XOR("text") });
		RegisterElement(&hitmarker, 1);

		hitsound.setup("hitsound", XOR("hitsound"), { XOR("off"), XOR("arena switch"), XOR("cod"), XOR("bubble"), XOR("bell"), XOR("star wars") });
		...
		hidden_cvar.setup(XOR("unlock hidden cvars"), XOR("hidden_cvars"));
		hidden_cvar.SetCallback(callbacks::UnlockHiddenConvars);
		RegisterElement(&hidden_cvar, 1);
	}
};
