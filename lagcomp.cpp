...
void LagCompensation::PredictAnimations( CCSGOPlayerAnimState* state, LagRecord* record ) {
	...
	backup.abs_veloctiy = player->m_vecAbsVelocity();
	backup.origin = player->m_vecOrigin();
	backup.abs_origin = player->GetAbsOrigin();
	...
	player->m_vecOrigin() = record->m_pred_origin;
	player->m_fFlags() = record->m_pred_flags;
	player->m_vecVelocity() = player->m_vecAbsVelocity() = record->m_pred_velocity;
	...
	player->m_vecAbsVelocity() = backup.abs_veloctiy;
	player->m_vecOrigin() = backup.origin;
	player->SetAbsOrigin(backup.abs_origin);
}