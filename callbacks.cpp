...
void callbacks::ToggleDownManual() {
	g_hvh.m_manual_side = 1;
}

void callbacks::ToggleLeftManual() {
	g_hvh.m_manual_side = 2;
}

void callbacks::ToggleRightManual() {
...
{
	if (!g_menu.main.misc.hidden_cvar.get())
		return;
...
bool callbacks::IsMultipointOn( ) {
	return g_menu.main.aimbot.static_multipoint_enable.get();
}
...
bool callbacks::IsFakeBodyCustomStand()
{
	return g_menu.main.antiaim.body_fake_stand.get ( ) == 1;
}

bool callbacks::IsFakeBodyCustomAir()
{
	return g_menu.main.antiaim.body_fake_air.get( ) == 1;
}
...
bool callbacks::IsFakeAntiAimSwap() {
	return g_menu.main.antiaim.fake_yaw.get() == 7;
}
...
bool callbacks::IsFakeAntiAimFlick()
{
	return g_menu.main.antiaim.fake_yaw.get() == 4;
}
