...
void Form::draw( ) {
	...
	if( !m_tabs.empty( ) ) {
		...
		render::rect(tabs_area.x + 1, tabs_area.y + 1, tabs_area.w - 2, tabs_area.h - 2, { 48, 48, 48, m_alpha }); //inner tab area border
		...
		if( !m_active_tab->m_elements.empty( ) ) {
			...
			render::rect(el.x + 1, el.y + 1, el.w - 2, el.h - 2, { 48, 48, 48, m_alpha }); //inner settings area border
